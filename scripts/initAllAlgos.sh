...
if (( incDBNum > 0 )); then
   days=$(getLastNDays $sym $incDBNum)
else
   days=$(getAllDays $sym)
fi
...
# Restrict the number of days to test when sym test data 
#  contains  days > maxNumDays
   
if [[ -n $debug ]]; then echo maxNumDays $maxNumDays; fi

if [[ -n $maxNumDays ]]; then
   days=$(getLastNDays $sym $maxNumDays)
#   totalNumDays=$(echo $days | grep -o ' ' | wc -l)
#   totalNumDays=$(echo "$totalNumDays + 1" | bc)
#   if [[ -n $debug ]]; then echo totalNumDays $totalNumDays; fi
#   if (( $totalNumDays > $maxNumDays )); then
#     # Reduce the set
#      firstDay=$(echo "$totalNumDays - $maxNumDays + 1" | bc)
#      days=$(echo $days | cut -f${firstDay}-${totalNumDays} -d' ')
#      if [[ -n $debug ]]; then echo firstDay $firstDay totDays $days; fi
#   fi
fi 
